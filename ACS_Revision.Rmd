...
All %>%  filter(density == 70, id %in% c("A","OrigA"), value_homophily == ethnic_homophily | id == "OrigA") %>%
	...
	geom_hline(aes(yintercept = baseline),
		data.frame(fraction_majority = c(50,60,70,80,90), baseline = c(0.5,0.5,0.5,0.5,0.5)), linetype="dotted",color="green") +
	...
	scale_color_manual(name="",labels=c("Ethnic Segregation","Value Segregation","Fraction happy","Segregation Original Schelling"), values=c("magenta","green", "black","magenta")) +
...
suppressMessages(
	All %>% filter(density == 70, id == "A", fraction_majority == 50, value_homophily == 30) %>%
		...
		scale_color_manual(values=c("gray","green", "magenta"), guide = guide_legend(override.aes = list(shape = c(46,19,15),
			linestyle=c("solid","solid","solid"),
			color = c("gray","green","magenta")), title ="" )) +
...
All %>% filter(density == 70, id == "A", fraction_majority == 80, value_homophily == 30) %>%
	...
	scale_color_manual(values=c("blue", "orange"), guide = guide_legend(title="")) + 
	scale_x_continuous(limits = c(0,1), breaks=c(0,0.3,0.6,1)) + 
