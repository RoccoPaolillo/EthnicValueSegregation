...
to setup
  ...
  ask patches [
    ...
    if random 100 < density [                           ;; density of entire population in society
      sprout 1 [
        set color ifelse-value (random 100 < (fraction_majority) ) [105] [26]    ;; population devided in ethnicity 1 (red) and 2 (green)
        if Version = "Extension" [
          set shape ifelse-value (random 100 < 50) ["circle"] ["square"]
          set color ifelse-value (shape = "circle") [color + 2] [color] ; Make color of circles a bit lighter to foster perception
        ]  ;; our extension: value-oriented agents (circle) / ethnicity-oriented agents (square)
...
to update-turtles
  ask turtles [
   ifelse first shape = "s" [ ;; homophily behavior of ethnicity-oriented agents, based on ethnicity = tag color
     set similar-nearby count (turtles-on neighbors) with [  abs (color - [ color ] of myself) <= 2  ] ; We have to check color difference instead of equality
     set other-nearby count (turtles-on neighbors) with [ abs (color - [ color ] of myself) > 2 ] ; We have to check color difference instead of equality
